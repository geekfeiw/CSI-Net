...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        self.generation = nn.Sequential(
            # 30*1*1 -> 256*2*2
            nn.ConvTranspose2d(30, 8 * self.unit_gen, kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(8 * self.unit_gen),
            nn.ReLU(),
            # 256*2*2 -> 4
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),

            # 4->7
            nn.ConvTranspose2d(8 * self.unit_gen, 4 * self.unit_gen, kernel_size=3, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(4 * self.unit_gen),
            nn.ReLU(),
            # 7->14
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),

            # 14->28->56
            nn.ConvTranspose2d(4 * self.unit_gen, 2 * self.unit_gen, kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(2 * self.unit_gen),
            nn.ReLU(),
            # 256*7*7 -> 128*14*14
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),

            nn.ConvTranspose2d(2 * self.unit_gen, self.unit_gen, kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(self.unit_gen),
            nn.ReLU(),
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(self.unit_gen, 64, kernel_size=7, stride=2, padding=3, bias=False),
            nn.BatchNorm2d(64),
